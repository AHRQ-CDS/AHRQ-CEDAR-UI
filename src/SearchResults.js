...
function SearchResults({ searchResults, page, onPageChange, onKeywordClick, onConceptClick }) {
    ...
    return (
        <React.Fragment>
          ...
          {searchResults.data.entry && 
            searchResults.data.entry.map(e => <SearchResult key={e.resource.id} resource={e.resource} onKeywordClick={onKeywordClick} onConceptClick={onConceptClick} />)
          }
        </React.Fragment>
    );
