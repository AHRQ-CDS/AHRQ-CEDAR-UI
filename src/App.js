...
function App(props) {
  ...
  const cedarSearch = async (conditionSearchString, additionalSearchString) => {
  ...
  useEffect(() => {
    cedarSearch(conditionSearchString, additionalSearchString);
  ...
  const handleConditionsChange = (conditionNames) => {
    ...
    const conditionSearchString = conditionNames.map(s => `"${s.replace(/\([^)]+\)/, '').replace(/[^\w\s]+/, '')}"`).join(' AND ');
    setConditionSearchString(conditionSearchString);
  ...
  const updateAdditionalSearchString = () => {
    setAdditionalSearchString(additionalSearchInput);
  };
        ...
        <Grid>
          <Grid.Row>
            <Grid.Column width={5}>
              ...
              <Segment>
                <h4>Filters</h4>
                <Input fluid placeholder='Search filter...' action={{ primary: true, icon: 'search', onClick: updateAdditionalSearchString }}
